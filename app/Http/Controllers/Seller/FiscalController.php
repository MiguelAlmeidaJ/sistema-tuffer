...
final class FiscalController extends Controller
{
    public function index(): string
    {
        ...
        $stmt = $pdo->prepare('SELECT s.*,u.email user_email FROM sellers s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.user_id=? LIMIT 1');
        $stmt->execute([$store['seller_id'], Auth::id()]);
        $seller = $stmt->fetch();
        if (!is_array($seller)) return Response::redirect('/vendedor');
        $cfg = new FiscalConfiguration();
        $profile = (new StoreFiscalProfileRepository($pdo, $cfg))->find((int) $store['id'], (int) $seller['id']);
        $stmt = $pdo->prepare('SELECT p.id,p.name,p.sku,p.status,pfp.* FROM products p LEFT JOIN product_fiscal_profiles pfp ON pfp.product_id=p.id WHERE p.store_id=? ORDER BY p.name LIMIT 250');
        $stmt->execute([$store['id']]);$products = $stmt->fetchAll();
        $stmt = $pdo->prepare('SELECT fd.*,so.code seller_order_code,o.code order_code FROM fiscal_documents fd JOIN seller_orders so ON so.id=fd.seller_order_id JOIN orders o ON o.id=so.order_id WHERE fd.store_id=? ORDER BY fd.id DESC LIMIT 50');
        $stmt->execute([$store['id']]);$documents = $stmt->fetchAll();
        return $this->page('seller/fiscal/index','layouts/seller',[
            ...
            'fiscalProvider'=>(string)($profile['provider']??$cfg->provider()),
            'fiscalEnvironment'=>(string)($profile['environment']??$cfg->environment()),
            'fiscalAutoIssue'=>(bool)($profile['auto_issue']??false),
        ]);
    }
    ...
    public function saveProfile(): string
    {
        ...
        $provider=mb_strtolower(trim((string)($_POST['provider']??'disabled')));$provider=preg_replace('/[^a-z0-9_-]+/','',$provider)??'disabled';$provider=mb_substr($provider===''?'disabled':$provider,0,60);
        if($mode===FiscalIssuanceMode::MANUAL)$provider='manual';elseif($mode===FiscalIssuanceMode::EXTERNAL&&$provider==='disabled')$provider='external';
        $environment=(string)($_POST['environment']??'homologation')==='production'?'production':'homologation';
        ...
        $enabled=!empty($_POST['enabled'])?1:0;$autoIssue=$mode===FiscalIssuanceMode::PLATFORM&&!empty($_POST['auto_issue'])?1:0;
        ...
        if($mode===FiscalIssuanceMode::PLATFORM){
            if($taxRegime===''||$crt===''||!preg_match('/^[A-Z]{2}$/',$state)||!preg_match('/^\d{7}$/',$ibge)||!preg_match('/^\d{8}$/',$cep)||$street===''||$number===''||$neighborhood===''||$city===''){Session::flash('error','Para emissão pela Tuffer, preencha regime, CRT e endereço fiscal completo com CEP, UF e código IBGE.');return Response::redirect('/vendedor/fiscal');}
            if($indicator==='contributor'&&$stateRegistration===''){Session::flash('error','Informe a inscrição estadual para uma loja contribuinte.');return Response::redirect('/vendedor/fiscal');}
        }else{$state=preg_match('/^[A-Z]{2}$/',$state)?$state:'';$ibge=preg_match('/^\d{7}$/',$ibge)?$ibge:'';$cep=preg_match('/^\d{8}$/',$cep)?$cep:'';}
        $sql='INSERT INTO store_fiscal_profiles(store_id,seller_id,enabled,issuance_mode,provider,environment,auto_issue,legal_name,trade_name,document,state_registration,tax_regime,crt,state_registration_indicator,municipal_registration,fiscal_email,postal_code,street,number,complement,neighborhood,city,state,city_ibge_code,nfe_series) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE seller_id=VALUES(seller_id),enabled=VALUES(enabled),issuance_mode=VALUES(issuance_mode),provider=VALUES(provider),environment=VALUES(environment),auto_issue=VALUES(auto_issue),legal_name=VALUES(legal_name),trade_name=VALUES(trade_name),document=VALUES(document),state_registration=VALUES(state_registration),tax_regime=VALUES(tax_regime),crt=VALUES(crt),state_registration_indicator=VALUES(state_registration_indicator),municipal_registration=VALUES(municipal_registration),fiscal_email=VALUES(fiscal_email),postal_code=VALUES(postal_code),street=VALUES(street),number=VALUES(number),complement=VALUES(complement),neighborhood=VALUES(neighborhood),city=VALUES(city),state=VALUES(state),city_ibge_code=VALUES(city_ibge_code),nfe_series=VALUES(nfe_series)';
        $pdo->prepare($sql)->execute([$store['id'],$store['seller_id'],$enabled,$mode,$provider,$environment,$autoIssue,$legalName,$tradeName!==''?$tradeName:null,$document,$stateRegistration!==''?$stateRegistration:null,$taxRegime!==''?$taxRegime:null,$crt!==''?$crt:null,$indicator,$municipalRegistration!==''?$municipalRegistration:null,$email!==''?$email:null,$cep!==''?$cep:null,$street!==''?$street:null,$number!==''?$number:null,$complement!==''?$complement:null,$neighborhood!==''?$neighborhood:null,$city!==''?$city:null,$state!==''?$state:null,$ibge!==''?$ibge:null,$series]);
        $this->resyncStore((int)$store['id']);Session::flash('success','Configuração fiscal desta loja salva. Pedidos pagos pendentes foram reprocessados pelo novo modo de emissão.');return Response::redirect('/vendedor/fiscal');
    }

    public function saveProduct(string $id): string
    {
        $productId=(int)$id;$context=new SellerStoreContext();$store=$context->current();if(!$store)return Response::redirect('/vendedor');$pdo=Database::connection();$stmt=$pdo->prepare('SELECT id FROM products WHERE id=? AND store_id=? AND seller_id=? LIMIT 1');$stmt->execute([$productId,$store['id'],$store['seller_id']]);if(!(int)$stmt->fetchColumn()){http_response_code(404);Session::flash('error','Produto não encontrado na loja atual.');return Response::redirect('/vendedor/fiscal');}
        $digits=static fn(string $field,int $max):?string=>(($value=substr(preg_replace('/\D+/','',(string)($_POST[$field]??''))??'',0,$max))===''?null:$value);$upper=static fn(string $field,int $max):?string=>(($value=mb_substr(mb_strtoupper(trim((string)($_POST[$field]??''))),0,$max))===''?null:$value);
    ...
    {
        $sellerOrderId=(int)$id;$context=new SellerStoreContext();$store=$context->current();if(!$store)return Response::redirect('/vendedor');$stmt=Database::connection()->prepare('SELECT id FROM seller_orders WHERE id=? AND store_id=? AND seller_id=? LIMIT 1');$stmt->execute([$sellerOrderId,$store['id'],$store['seller_id']]);if(!(int)$stmt->fetchColumn()){http_response_code(404);Session::flash('error','Pedido fiscal não encontrado na loja atual.');return Response::redirect('/vendedor/fiscal');}
        try{(new FiscalOrchestratorService())->registerOutsideDocument($sellerOrderId,['number'=>$_POST['number']??null,'series'=>$_POST['series']??null,'access_key'=>$_POST['access_key']??null,'protocol'=>$_POST['protocol']??null,'external_reference'=>$_POST['external_reference']??null]);Session::flash('success','NF-e vinculada ao pedido da loja.');}
    ...
    {
        $stmt=Database::connection()->prepare("SELECT id FROM seller_orders WHERE store_id=? AND status IN ('paid','processing','shipped','delivered') ORDER BY id DESC LIMIT 100");$stmt->execute([$storeId]);$service=new FiscalOrchestratorService();foreach($stmt->fetchAll(PDO::FETCH_COLUMN) as $id){try{$service->syncSellerOrder((int)$id);}catch(Throwable){}}
    }
}
