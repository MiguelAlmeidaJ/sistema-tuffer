...
final class AddressController extends Controller
{
    ...
    private function save(?int $id=null): void{$pdo=Database::connection();$default=!empty($_POST['is_default'])?1:0;if($default)$pdo->prepare('UPDATE user_addresses SET is_default=0 WHERE user_id=?')->execute([Auth::id()]);$ibge=preg_replace('/\D+/','',(string)($_POST['city_ibge_code']??''))??'';$ibge=preg_match('/^\d{7}$/',$ibge)?$ibge:null;$v=[trim((string)($_POST['label']??'')),trim((string)$_POST['recipient_name']),trim((string)$_POST['postal_code']),trim((string)$_POST['street']),trim((string)$_POST['number']),trim((string)($_POST['complement']??'')),trim((string)$_POST['neighborhood']),trim((string)$_POST['city']),$ibge,mb_strtoupper(trim((string)$_POST['state'])),$default];if($id){$v[]=$id;$v[]=Auth::id();$pdo->prepare('UPDATE user_addresses SET label=?,recipient_name=?,postal_code=?,street=?,number=?,complement=?,neighborhood=?,city=?,city_ibge_code=?,state=?,is_default=? WHERE id=? AND user_id=?')->execute($v);}else{array_unshift($v,Auth::id());$pdo->prepare('INSERT INTO user_addresses(user_id,label,recipient_name,postal_code,street,number,complement,neighborhood,city,city_ibge_code,state,is_default) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)')->execute($v);}}
    ...
    private function safeReturn(string $path): string{if($path===''||!str_starts_with($path,'/')||str_starts_with($path,'//'))return'';if(str_contains($path,'\\')||preg_match('/[\x00-\x1F\x7F]/',$path)===1)return'';return $path;}
}
