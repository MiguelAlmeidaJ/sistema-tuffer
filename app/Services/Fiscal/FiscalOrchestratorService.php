...
final class FiscalOrchestratorService
{
    ...
    {
        return (new ExternalFiscalDocumentService($this->pdo, $this))->authorize($sellerOrderId, $data);
    }
    ...
    private function prepareStoreManagedDocument(array $document, array $context, array $profile, string $mode): array
    {
        ...
        try {
            $this->pdo->prepare('UPDATE fiscal_documents SET issuance_mode=?,provider=?,environment=?,series=?,products_total=?,shipping_total=?,discount_total=?,grand_total=?,issuer_snapshot=?,recipient_snapshot=?,tax_context=?,validation_errors=NULL,status=?,requires_action=1,action_reason=?,error_code=NULL,error_message=NULL WHERE id=?')->execute([
                ...
                $this->json(['issuer_state' => $issuerState, 'destination_state' => $destinationState, 'managed_by_store' => true, 'tuffer_emits_document' => false]),
                $status,
                $reason,
                $document['id'],
            ]);
    ...
    private function findOrCreateDocument(array $context, ?array $profile): array
    {
        ...
        $this->pdo->prepare("INSERT INTO fiscal_documents(seller_order_id,seller_id,store_id,document_type,operation_type,issuance_mode,revision,provider,environment,idempotency_key,status,series,products_total,shipping_total,discount_total,grand_total) VALUES(?,?,?,'nfe','sale',?,1,?,?,?,'pending',?,?,?,?,?)")->execute([
            $context['seller_order_id'], $context['seller_id'], $context['store_id'], $mode, $provider, $environment, $key,
            (int) ($profile['nfe_series'] ?? 1), $context['products_total'], $context['shipping_total'], $context['discount_total'], $this->grand($context),
        ]);
        $id = (int) $this->pdo->lastInsertId();
        $this->event($id, 'created', 'pending', 'Acompanhamento fiscal criado a partir do pedido pago.', ['seller_order_id' => (int) $context['seller_order_id'], 'mode' => $mode]);
    ...
    private function replaceItems(int $documentId, array $items, string $issuerState, string $destinationState): void
    {
        ...
        foreach ($items as $item) {
            ...
            $insert->execute([
                $documentId, $item['order_item_id'], $item['product_id'], $item['product_variant_id'], $item['product_name'], $item['sku'], $item['quantity'], $item['unit_price'], $item['total'],
                $item['ncm'] ?: null, $item['cest'] ?: null, $item['origin'], $item['commercial_unit'] ?: null, $item['tributary_unit'] ?: null, $item['gtin'] ?: null,
                $item['cfop_in_state'] ?: null, $item['cfop_out_state'] ?: null, $item[$sameState ? 'cfop_in_state' : 'cfop_out_state'] ?: null,
                $item['icms_code'] ?: null, $item['pis_code'] ?: null, $item['cofins_code'] ?: null, $item['ipi_code'] ?: null, $item['ibs_cbs_cst'] ?: null, $item['ibs_cbs_classification'] ?: null, $meta,
            ]);
        }
    }
    ...
    {
        return [
            'seller_id' => (int) $context['seller_id'], 'store_id' => (int) $context['store_id'],
            'legal_name' => $profile['legal_name'] ?? $context['legal_name'], 'trade_name' => $profile['trade_name'] ?? $context['trade_name'],
            'document' => $profile['document'] ?? $context['document'], 'state_registration' => $profile['state_registration'] ?? $context['state_registration'],
            'state_registration_indicator' => $profile['state_registration_indicator'] ?? 'contributor', 'municipal_registration' => $profile['municipal_registration'] ?? null,
            'tax_regime' => $profile['tax_regime'] ?? null, 'crt' => $profile['crt'] ?? null, 'fiscal_email' => $profile['fiscal_email'] ?? null,
            'address' => ['postal_code' => $profile['postal_code'] ?? null, 'street' => $profile['street'] ?? null, 'number' => $profile['number'] ?? null, 'complement' => $profile['complement'] ?? null, 'neighborhood' => $profile['neighborhood'] ?? null, 'city' => $profile['city'] ?? null, 'city_ibge_code' => $profile['city_ibge_code'] ?? null, 'state' => $profile['state'] ?? null],
        ];
    }
    ...
    {
        return [
            'user_id' => (int) $context['user_id'], 'name' => $context['recipient_name'] ?: $context['customer_name'], 'document' => $context['customer_document'], 'email' => $context['customer_email'], 'phone' => $context['customer_phone'],
            'address' => ['postal_code' => $context['destination_postal_code'], 'street' => $context['destination_street'], 'number' => $context['destination_number'], 'complement' => $context['destination_complement'], 'neighborhood' => $context['destination_neighborhood'], 'city' => $context['destination_city'], 'city_ibge_code' => $context['destination_city_ibge_code'], 'state' => $context['destination_state']],
        ];
    }
    ...
    private function event(int $documentId, string $type, string $status, ?string $message, array $payload = []): void
    {
        ...
        $this->pdo->prepare('INSERT INTO fiscal_events(fiscal_document_id,event_type,status,message,payload_sha256) VALUES(?,?,?,?,?)')->execute([
            $documentId, mb_substr($type, 0, 60), mb_substr($status, 0, 30), $message === null ? null : mb_substr($message, 0, 1000), $hash,
        ]);
    }
