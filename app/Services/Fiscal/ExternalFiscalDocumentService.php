...
final class ExternalFiscalDocumentService
{
    ...
    private function assertOutsideMode(array $document): void
    {
        $mode = FiscalIssuanceMode::normalize((string) ($document['issuance_mode'] ?? 'manual'));
    ...
    private function event(int $documentId, string $type, string $status, string $message, array $payload = []): void
    {
        ...
        $encoded = $payload === [] ? null : json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR);
        ...
        $this->pdo->prepare('INSERT INTO fiscal_events(fiscal_document_id,event_type,status,message,payload_sha256) VALUES(?,?,?,?,?)')
            ->execute([$documentId, mb_substr($type,0,60), mb_substr($status,0,30), mb_substr($message,0,1000), $hash]);
    }
}
