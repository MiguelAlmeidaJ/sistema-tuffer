...
final class TinyFiscalConnectorService
{
    ...
    public function process(int $runId):void
    {
        ...
        try{
            ...
            if($link===''){try{$link=(string)($client->getInvoiceLink($providerDocumentId)??'');}catch(Throwable){}}
            ...
            if(in_array($situation,[5,10],true)){
                ...
                $this->markFailure((int)$config['id'],$message);return;
            }
            ...
            if(!$authorized||!preg_match('/^\d{44}$/',$accessKey)){
                if(!(bool)$config['auto_emit']){
                    ...
                    $pdo->prepare("UPDATE fiscal_documents SET status='processing',requires_action=1,action_reason=?,error_code=NULL,error_message=NULL WHERE id=? AND status<>'authorized'")->execute([$message,$run['fiscal_document_id']]);return;
                }
            ...
            if($documentNumber===''||$documentSeries===''){
                $detail=$client->getInvoice($providerDocumentId);$documentNumber=(string)($detail['numero']??$documentNumber);$documentSeries=(string)($detail['serie']??$documentSeries);
                if($xml===null&&is_string($detail['xml']??null)&&trim((string)$detail['xml'])!=='')$xml=(string)$detail['xml'];
            }
            $number=(int)preg_replace('/\D+/','',$documentNumber);$series=(int)preg_replace('/\D+/','',$documentSeries);
            if($number<1||$series<1)throw new RuntimeException('Tiny autorizou a NF-e, mas não retornou número e série válidos.');
            (new ExternalFiscalDocumentService($pdo))->authorize((int)$run['seller_order_id'],['number'=>$number,'series'=>$series,'access_key'=>$accessKey,'xml'=>$xml,'external_reference'=>'tiny:nfe:'.$providerDocumentId]);
        ...
        }catch(FiscalConnectorPendingException $e){$pdo->prepare('UPDATE fiscal_connector_runs SET last_error=NULL WHERE id=?')->execute([$runId]);throw $e;}
        catch(Throwable $e){$message=mb_substr($e->getMessage(),0,1000);$pdo->prepare("UPDATE fiscal_connector_runs SET last_error=? WHERE id=? AND status NOT IN ('completed','rejected')")->execute([$message,$runId]);$this->markFailure((int)$config['id'],$message);throw $e;}
    }

    public function enqueueEligibleStoreOrders(int $storeId,int $sellerId):void
    {
        $stmt=$this->pdo()->prepare("SELECT id FROM seller_orders WHERE store_id=? AND seller_id=? AND status IN ('paid','processing','shipped','delivered') ORDER BY id DESC LIMIT 100");$stmt->execute([$storeId,$sellerId]);$orchestrator=new FiscalOrchestratorService($this->pdo());
        foreach($stmt->fetchAll(PDO::FETCH_COLUMN) as $sellerOrderId){try{$orchestrator->syncSellerOrder((int)$sellerOrderId);$this->enqueue((int)$sellerOrderId);}catch(Throwable){}}
    }
    ...
    {
        $id=(int)($run['provider_order_id']??0);$number=(string)($run['provider_order_number']??'');if($id>0)return[$id,$number];
        $found=$client->findOrderByEcommerce($code);$order=$found??$client->createOrder($this->buildOrder($payload));$id=(int)($order['id']??0);$number=(string)($order['numero']??'');
        if($id<1)throw new RuntimeException('Tiny não retornou o ID do pedido para sincronização fiscal.');
        $this->pdo()->prepare("UPDATE fiscal_connector_runs SET status='order_synced',provider_order_id=?,provider_order_number=? WHERE id=?")->execute([(string)$id,$number!==''?$number:null,$runId]);return[$id,$number];
    }
    ...
    {
        $id=(int)($run['provider_document_id']??0);$number=(string)($run['provider_document_number']??'');$series=(string)($run['provider_document_series']??'');if($id>0)return[$id,$number,$series];
        $invoice=$client->findInvoiceByEcommerce($code);
        if(is_array($invoice)&&(int)($invoice['id']??0)>0){$id=(int)$invoice['id'];$number=(string)($invoice['numero']??'');$series=(string)($invoice['serie']??'');}
        else{$generated=$client->generateInvoice($providerOrderId);$id=(int)($generated['idNotaFiscal']??0);$number=(string)($generated['numero']??'');$series=(string)($generated['serie']??'');}
        if($id<1)throw new RuntimeException('Tiny não retornou o ID da NF-e gerada.');
        $this->pdo()->prepare("UPDATE fiscal_connector_runs SET status='invoice_generated',provider_document_id=?,provider_document_number=?,provider_document_series=? WHERE id=?")->execute([(string)$id,$number!==''?$number:null,$series!==''?$series:null,$runId]);return[$id,$number,$series];
    }
    ...
    private function buildOrder(array $payload):array
    {
        ...
        foreach($payload['items'] as $item){$unit=mb_substr((string)($item['fiscal']['commercial_unit']??'UN'),0,3)?:'UN';$items[]=['item'=>['codigo'=>mb_substr((string)($item['sku']??''),0,60),'descricao'=>mb_substr((string)$item['name'],0,120),'unidade'=>$unit,'quantidade'=>(string)$item['quantity'],'valor_unitario'=>(string)$item['unit_price']]];}
        if($items===[])throw new RuntimeException('Pedido sem itens para sincronizar com Tiny.');
        $document=trim((string)($recipient['document']??''));$digits=preg_replace('/\D+/','',$document)??'';$created=(string)($payload['seller_order']['created_at']??$payload['seller_order']['order_created_at']??'');$timestamp=$created!==''?strtotime($created):false;
        $order=['data_pedido'=>date('d/m/Y',$timestamp!==false?$timestamp:time()),'cliente'=>['nome'=>mb_substr((string)$recipient['name'],0,30),'tipo_pessoa'=>strlen($digits)===14?'J':'F','cpf_cnpj'=>$document,'endereco'=>mb_substr((string)($address['street']??''),0,50),'numero'=>mb_substr((string)($address['number']??''),0,10),'complemento'=>mb_substr((string)($address['complement']??''),0,50),'bairro'=>mb_substr((string)($address['neighborhood']??''),0,30),'cep'=>mb_substr((string)($address['postal_code']??''),0,10),'cidade'=>mb_substr((string)($address['city']??''),0,30),'uf'=>mb_substr((string)($address['state']??''),0,2),'pais'=>'Brasil','fone'=>mb_substr((string)($recipient['phone']??''),0,40),'email'=>mb_substr((string)($recipient['email']??''),0,50),'atualizar_cliente'=>'S'],'itens'=>$items,'valor_frete'=>(string)$amounts['shipping_total'],'valor_desconto'=>(string)$amounts['discount_total'],'situacao'=>'aprovado','numero_pedido_ecommerce'=>mb_substr((string)$payload['seller_order']['code'],0,50),'ecommerce'=>'Tuffer','obs_internas'=>mb_substr('Pedido marketplace Tuffer '.$payload['seller_order']['order_code'],0,100)];
        $intermediaryName=trim((string)($_ENV['FISCAL_INTERMEDIARY_NAME']??''));$intermediaryDocument=trim((string)($_ENV['FISCAL_INTERMEDIARY_DOCUMENT']??''));
        if($intermediaryName!==''&&$intermediaryDocument!==''){$order['intermediador']=['nome'=>mb_substr($intermediaryName,0,60),'cnpj'=>mb_substr($intermediaryDocument,0,18)];$paymentDocument=trim((string)($_ENV['FISCAL_PAYMENT_INTERMEDIARY_DOCUMENT']??''));if($paymentDocument!=='')$order['intermediador']['cnpjPagamento']=mb_substr($paymentDocument,0,18);}
        return$order;
    }

    /** @return array<string,mixed> */
    private function storeProfile(int $storeId,int $sellerId):array{$stmt=$this->pdo()->prepare('SELECT * FROM store_fiscal_profiles WHERE store_id=? AND seller_id=? LIMIT 1');$stmt->execute([$storeId,$sellerId]);$row=$stmt->fetch();if(!is_array($row))throw new RuntimeException('Salve primeiro os dados fiscais desta loja antes de conectar o Tiny.');return$row;}
    /** @param array<string,mixed> $profile @param array<string,mixed> $account */
    private function assertAccountMatchesStore(array $profile,array $account):void{$store=$this->normalizeDocument((string)($profile['document']??''));$tiny=$this->normalizeDocument((string)($account['cnpj_cpf']??''));if($store!==''&&$tiny!==''&&!hash_equals($store,$tiny))throw new RuntimeException('O CPF/CNPJ da conta Tiny não corresponde ao documento fiscal desta loja. Conecte a conta Tiny correta.');}
    private function normalizeDocument(string $value):string{return preg_replace('/[^A-Z0-9]+/','',mb_strtoupper(trim($value)))??'';}
    /** @return array<string,mixed> */
    private function configurationWithSecret(int $storeId,int $sellerId):array{$stmt=$this->pdo()->prepare("SELECT * FROM store_fiscal_connector_configs WHERE store_id=? AND seller_id=? AND provider='tiny' AND enabled=1 LIMIT 1");$stmt->execute([$storeId,$sellerId]);$row=$stmt->fetch();if(!is_array($row))throw new RuntimeException('Conector Tiny não está ativo nesta loja.');return$row;}
    /** @param array<string,mixed> $config */
    private function client(array $config):TinyApiClient{$credentials=$this->credentialVault()->decrypt((string)$config['credentials_ciphertext']);return new TinyApiClient((string)($credentials['token']??''));}
    /** @return array<string,mixed> */
    private function run(int $runId):array{$stmt=$this->pdo()->prepare('SELECT * FROM fiscal_connector_runs WHERE id=? AND provider=? LIMIT 1');$stmt->execute([$runId,self::PROVIDER]);$row=$stmt->fetch();if(!is_array($row))throw new RuntimeException('Execução Tiny não encontrada.');return$row;}
    private function markFailure(int $configId,string $message):void{$this->pdo()->prepare('UPDATE store_fiscal_connector_configs SET last_failure_at=NOW(),last_error=? WHERE id=?')->execute([mb_substr($message,0,1000),$configId]);}
